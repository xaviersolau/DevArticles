...
namespace FctCodeGen.Patterns.Impl
{
    ...
    public class OrchestrationPatternClient : OrchestrationClientBase, IOrchestrationPattern
    {
        ...
        {
            var payload = new OrchestrationPatternFunction.MethodPatternPayload
            {
                Argument = Repeat.Affectation("argument", argument),
            };

            return ScheduleNewOrchestrationInstanceAsync<ReturnType, OrchestrationPatternFunction.MethodPatternPayload>(nameof(IOrchestrationPattern) + nameof(IOrchestrationPattern.MethodPattern), payload);
        }
    }
}
